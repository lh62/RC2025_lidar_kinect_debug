...
// Your Project Includes - 您的项目头文件 (确保它们在 include/yolo_realsense/ 目录下)
#include <fstream>
#include <iostream>
#include <chrono>
#include <opencv2/opencv.hpp>
...
#include "yolo_realsense/DetectedObject3D.h"
#include "yolo_realsense/DetectedObject3DArray.h"
...
private:
    ...
    void deserialize_engine(std::string& engine_name, IRuntime** runtime, ICudaEngine** engine, IExecutionContext** context) {
        ...
        if (!file.good()) {
            ROS_ERROR("读取 %s 失败!", engine_name.c_str());
        ...
        *context = (*engine)->createExecutionContext();
        ...
        ROS_INFO("引擎文件 %s 反序列化成功。", engine_name.c_str());
    }
    ...
    void prepare_buffer(ICudaEngine* engine, float** input_buffer_device, float** output_buffer_device,
        ...
        if (cuda_post_process == "c") {
            *output_buffer_host = new float[kBatchSize * kOutputSize];
            ...
            if (kBatchSize > 1) {
                 ROS_ERROR("GPU 后处理尚不支持大于 1 的批处理大小。");
                 ros::shutdown();
            }
            *decode_ptr_host = new float[1 + kMaxNumOutputBbox * bbox_element];
            CUDA_CHECK(cudaMalloc((void**)decode_ptr_device, sizeof(float) * (1 + kMaxNumOutputBbox * bbox_element)));
        }
         ROS_INFO("GPU/CPU 缓冲区准备完毕。");
    }
    ...
    void infer(IExecutionContext& context, cudaStream_t& stream, void** buffers, float* output, int batchsize,
        ...
        ROS_DEBUG("推理和后处理时间: %ld ms", std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count());
    }
...
public:
      ...
      sync_(MySyncPolicy(10), color_sub_, depth_sub_, info_sub_) // 初始化同步器，队列大小为 10
    {
        ...
        // --- 设置 ROS 发布器 ---
        pub_detections_3d_ = nh_.advertise<yolo_realsense::DetectedObject3DArray>("/yolo/detections_3d", 10);
        pub_markers_ = nh_.advertise<visualization_msgs::MarkerArray>("/yolo/markers", 10);
        pub_debug_image_ = it_.advertise("/yolo/debug_image", 1);
        ...
        ROS_INFO("YOLOv11 ROS node initialized successfully. Subscribed to the following topics:\n\tColor: %s\n\tDepth: %s\n\tInfo: %s",
    ...
    ~YoloDetectorNode() {
        ...
        delete engine_;
        delete runtime_;
        ROS_INFO("YOLOv11 ROS node has been shut down and resources released.");
    }
};
